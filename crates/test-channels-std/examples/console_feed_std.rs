...
fn main() {
    ...
    let (tx_slow, rx_slow) = hotpath::channel!(
        std::sync::mpsc::sync_channel::<String>(5),
        label = "slow-consumer",
        capacity = 5,
        log = true
    );
    ...
    let (tx_burst, rx_burst) = hotpath::channel!(
        std::sync::mpsc::sync_channel::<u64>(10),
        label = "burst-traffic",
        capacity = 10,
        log = true
    );
    ...
    let (tx_gradual, rx_gradual) = hotpath::channel!(
        std::sync::mpsc::sync_channel::<f64>(20),
        label = "gradual-flow",
        capacity = 20,
        log = true
    );
    ...
    for i in 0..3 {
        let (tx, rx) = hotpath::channel!(std::sync::mpsc::sync_channel::<u32>(5), capacity = 5);
