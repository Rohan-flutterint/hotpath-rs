...
impl<'a> MetricsProvider<'a> for TimingStatsData<'a> {
    ...
    fn metric_data(&self) -> HashMap<String, Vec<MetricType>> {
        ...
        let grand_total_duration_ns: u64 = self
            .stats
            .iter()
            .filter(|(_, s)| s.has_data)
            .map(|(_, stats)| stats.total_duration_ns)
            .sum();

        filtered_stats
            .into_iter()
            .map(|(function_name, stats)| {
                let percentage = if grand_total_duration_ns > 0 {
                    (stats.total_duration_ns as f64 / grand_total_duration_ns as f64) * 100.0
                } else {
                    0.0
                };
